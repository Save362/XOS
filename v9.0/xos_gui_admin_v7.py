...
class XOS(tk.Tk):
    ...
    def open_start_center(self):
        ...
        apps = [
            ...
            ("⚙️ XSettings", self.open_xsettings),
            ("🖥️ XTaskManager", self.open_task_manager),
            ("🔔 Bildirim Merkezi", self.open_notification_center),
            ("🔒 Kilit Ekranı", self.lock_xos),
            ("🎨 XPaint", self.open_xpaint),
            ("🛒 XStore", self.open_xstore),
    ...
    def _build_taskbar(self):
        ...
        notify_btn.pack(side="right", padx=(4, 0), pady=8)

        lock_btn = tk.Button(
            self.taskbar,
            text="🔒",
            command=self.lock_xos,
            bg=TASKBAR_BG,
            fg=TASKBAR_FG,
            activebackground="#374151",
            activeforeground="white",
        ...
        lock_btn.pack(side="right", padx=2, pady=8)
    ...
    def open_xupdate_tool(self):
        win=self._new_panel("XUpdateTool",560,520); self.update_idletasks()
        win.place(x=max(10,(self.desktop.winfo_width()-560)//2),y=max(10,(self.desktop.winfo_height()-520)//2),width=560,height=520)
        c=win.content
        tk.Label(c,text="🔄 XUpdateTool",bg=WINDOW_BG,font=("Segoe UI",20,"bold")).pack(pady=(22,4))
        tk.Label(c,text="GitHub Releases üzerinden X OS güncellemelerini kontrol eder.",bg=WINDOW_BG,fg="#6b7280").pack()
        tk.Label(c,text=f"Yüklü sürüm: v{XOS_VERSION}",bg=WINDOW_BG,font=("Segoe UI",10,"bold")).pack(pady=(12,4))
        status=tk.Label(c,text="Kontrol edilmeye hazır.",bg=WINDOW_BG,fg="#374151",wraplength=490,justify="center"); status.pack(pady=10)
        result=tk.Text(c,height=8,width=60,state="disabled",wrap="word"); result.pack(padx=12,pady=5)
        def show_result(v):
            try: result.config(state="normal"); result.delete("1.0","end"); result.insert("1.0",v); result.config(state="disabled")
            except tk.TclError: pass
        actions=tk.Frame(c,bg=WINDOW_BG); actions.pack(pady=6)
        def install_update(url,latest):
            import os,sys,tempfile,subprocess,urllib.request,py_compile
            check_btn.config(state="disabled"); update_btn.config(state="disabled"); status.config(text=f"v{latest} indiriliyor..."); show_result("Yeni X OS dosyası indiriliyor...")
            def worker():
                tmp=None
                try:
                    target=Path(__file__).resolve(); fd,name=tempfile.mkstemp(prefix="xos_update_",suffix=".py"); os.close(fd); tmp=Path(name)
                    req=urllib.request.Request(url,headers={"User-Agent":"XUpdateTool-XOS"})
                    with urllib.request.urlopen(req,timeout=60) as r: data=r.read()
                    if not data or len(data)<1000: raise RuntimeError("İndirilen X OS dosyası geçersiz veya boş.")
                    tmp.write_bytes(data); py_compile.compile(str(tmp),doraise=True)
                    installer=tmp.with_name(tmp.stem+"_installer.py"); installer.write_text('import sys,time,shutil,subprocess\nfrom pathlib import Path\ntarget=Path(sys.argv[1]).resolve(); new=Path(sys.argv[2]).resolve(); backup=Path(sys.argv[3]).resolve()\ntime.sleep(2.5)\ntry:\n    if target.exists(): shutil.copy2(target,backup)\n    shutil.copy2(new,target)\n    subprocess.Popen([sys.executable,str(target)],cwd=str(target.parent),close_fds=True)\nexcept Exception:\n    try:\n        if backup.exists():\n            shutil.copy2(backup,target)\n            subprocess.Popen([sys.executable,str(target)],cwd=str(target.parent),close_fds=True)\n    except Exception: pass\ntry: new.unlink(missing_ok=True); Path(__file__).unlink(missing_ok=True)\nexcept Exception: pass\n',encoding="utf-8")
                    backup=Path(str(target)+".backup")
                    subprocess.Popen([sys.executable,str(installer),str(target),str(tmp),str(backup)],cwd=str(target.parent),creationflags=getattr(subprocess,"CREATE_NO_WINDOW",0),close_fds=True)
                    self.after(0,lambda:(status.config(text=f"v{latest} kuruluyor..."),show_result(f"v{latest} doğrulandı ve kuruluma hazır.\n\nX OS yeniden başlatılıyor..."),self.after(1000,self.destroy)))
                    ...
                    err=str(exc)
                    def fail():
                        try: check_btn.config(state="normal"); update_btn.config(state="normal"); status.config(text="Güncelleme kurulamadı."); show_result("Yeni X OS dosyası doğrulanamadı veya kurulamadı.\n\nHata: "+err)
                        except tk.TclError: pass
                    self.after(0,fail)
            import threading; threading.Thread(target=worker,daemon=True).start()
        def check_github():
            check_btn.config(state="disabled"); status.config(text="GitHub'da güncelleme aranıyor..."); show_result("GitHub Releases kontrol ediliyor...")
            def worker():
                import json,urllib.request
                try:
                    url=f"https://api.github.com/repos/{XOS_GITHUB_OWNER}/{XOS_GITHUB_REPO}/releases/latest"; req=urllib.request.Request(url,headers={"Accept":"application/vnd.github+json","User-Agent":"XUpdateTool-XOS"})
                    with urllib.request.urlopen(req,timeout=15) as r: data=json.loads(r.read().decode("utf-8"))
                    latest=str(data.get("tag_name","")).strip().lstrip("vV"); notes=str(data.get("body","")).strip() or "Değişiklik notu yok."; asset_url=""
                    for a in data.get("assets",[]):
                        if str(a.get("name",""))==XOS_RELEASE_ASSET: asset_url=str(a.get("browser_download_url","")); break
                    if not asset_url:
                        for a in data.get("assets",[]):
                            if str(a.get("name","")).lower().endswith(".py"): asset_url=str(a.get("browser_download_url","")); break
                    def ver(v): return tuple(int(x) for x in v.split(".") if x.isdigit())
                    newer=bool(latest) and ver(latest)>ver(XOS_VERSION)
                    def done():
                        try:
                            check_btn.config(state="normal")
                            if not latest: status.config(text="GitHub sürüm bilgisi okunamadı."); return
                            if not newer: update_btn.config(state="disabled",text="✓ Güncelleme yok"); status.config(text=f"✓ X OS güncel — v{XOS_VERSION}"); show_result(f"GitHub'daki son yayın: v{latest}\n\nYeni güncelleme bulunamadı."); return
                            if not asset_url: status.config(text=f"v{latest} bulundu."); show_result(f"v{latest} bulundu fakat Release içinde .py dosyası bulunamadı."); return
                            status.config(text=f"🆕 Yeni sürüm bulundu: v{latest}"); show_result(f"Mevcut sürüm: v{XOS_VERSION}\nYeni sürüm: v{latest}\n\nDeğişiklikler:\n{notes}"); update_btn.config(state="normal",text=f"⬇ Güncelle ve v{latest}'a Geç",command=lambda:install_update(asset_url,latest))
                        except tk.TclError: pass
                    self.after(0,done)
                except Exception as exc:
                    err=str(exc); self.after(0,lambda: (check_btn.config(state="normal"),status.config(text="Güncelleme kontrolü başarısız."),show_result("GitHub'a bağlanılamadı.\n\nHata: "+err)))
            import threading; threading.Thread(target=worker,daemon=True).start()
        check_btn=tk.Button(actions,text="🔎 Güncellemeleri Kontrol Et",command=check_github,bg=ACCENT,fg="white",relief="flat",font=("Segoe UI",10,"bold"),padx=18,pady=9); check_btn.pack(pady=4)
        update_btn=tk.Button(actions,text="⬇ Güncelleme bulunamadı",command=lambda:None,bg="#9ca3af",fg="white",relief="flat",font=("Segoe UI",10,"bold"),padx=18,pady=9,state="disabled"); update_btn.pack(pady=4)
        tk.Label(c,text=f"GitHub: {XOS_GITHUB_OWNER}/{XOS_GITHUB_REPO}",bg=WINDOW_BG,fg="#9ca3af",font=("Segoe UI",8)).pack(pady=4)

    def open_xsettings(self):
        win = self._new_panel("⚙️ XSettings", 520, 430)
        c = win.content

        tk.Label(
            c, text="⚙️ XSettings",
            bg=WINDOW_BG, font=("Segoe UI", 20, "bold")
        ).pack(pady=(20, 4))

        tk.Label(
            c, text="X OS görünümünü ve bazı davranışlarını değiştir.",
            bg=WINDOW_BG, fg="#6b7280"
        ).pack(pady=(0, 15))

        # Theme
        tk.Label(
            c, text="Tema", bg=WINDOW_BG,
            font=("Segoe UI", 10, "bold")
        ).pack(anchor="w", padx=25)

        theme = tk.StringVar(value=self.xos_theme)
        theme_box = tk.Frame(c, bg=WINDOW_BG)
        theme_box.pack(fill="x", padx=25, pady=5)

        def apply_theme():
            self.xos_theme = theme.get()
            if self.xos_theme == "Açık":
                desktop_color = "#dbeafe"
                fg = "#111827"
                taskbar_color = "#e5e7eb"
                taskbar_fg = "#111827"
            else:
                desktop_color = "#0f2740"
                fg = "#ffffff"
                taskbar_color = "#111827"
                taskbar_fg = "#e5e7eb"

            self.wallpaper = desktop_color
            global BG_DESKTOP, TASKBAR_BG, TASKBAR_FG
            BG_DESKTOP = desktop_color
            TASKBAR_BG = taskbar_color
            TASKBAR_FG = taskbar_fg

            try:
                self.configure(bg=desktop_color)
                self.desktop.config(bg=desktop_color)
                self.taskbar.config(bg=taskbar_color)

                for widget in self.desktop.winfo_children():
                    try:
                        widget.config(bg=desktop_color)
                        for child in widget.winfo_children():
                            try:
                                child.config(bg=desktop_color, fg=fg)
                            except tk.TclError:
                                pass
                    except tk.TclError:
                        pass
                ...
                for widget in self.taskbar.winfo_children():
                    try:
                        widget.config(bg=taskbar_color, fg=taskbar_fg)
                    except tk.TclError:
                        pass
            except tk.TclError:
                pass
            ...
            self.add_notification(
                f"Tema değiştirildi: {self.xos_theme}"
            )

        tk.Radiobutton(
            theme_box, text="🌙 Koyu", variable=theme, value="Koyu",
            command=apply_theme, bg=WINDOW_BG
        ).pack(side="left", padx=10)
        tk.Radiobutton(
            theme_box, text="☀️ Açık", variable=theme, value="Açık",
            command=apply_theme, bg=WINDOW_BG
        ).pack(side="left", padx=10)

        # Wallpaper presets
        tk.Label(
            c, text="Duvar kağıdı rengi", bg=WINDOW_BG,
            font=("Segoe UI", 10, "bold")
        ).pack(anchor="w", padx=25, pady=(15, 4))

        wall = tk.StringVar(value=self.wallpaper)
        wall_frame = tk.Frame(c, bg=WINDOW_BG)
        wall_frame.pack(fill="x", padx=25)
        ...
        def apply_wallpaper():
            global BG_DESKTOP
            BG_DESKTOP = wall.get()
            self.wallpaper = wall.get()
            try:
                self.desktop.config(bg=self.wallpaper)
                for widget in self.desktop.winfo_children():
                    try:
                        widget.config(bg=self.wallpaper)
                        for child in widget.winfo_children():
                            try:
                                child.config(bg=self.wallpaper)
                            except tk.TclError:
                                pass
                    except tk.TclError:
                        pass
                ...
                pass
            self.add_notification("Duvar kağıdı rengi değiştirildi.")

        for label, value in wallpapers:
            tk.Radiobutton(
                wall_frame, text=label, variable=wall, value=value,
                command=apply_wallpaper, bg=WINDOW_BG
            ).pack(side="left", padx=4)

        tk.Button(
            c, text="🔔 Bildirimleri Temizle",
        ...
        tk.Label(
            c,
            text="Değişiklikler X OS oturumu boyunca uygulanır.",
    ...
    def open_task_manager(self):
        win = self._new_panel("🖥️ XTaskManager", 520, 410)
        c = win.content

        tk.Label(
            c, text="🖥️ XTaskManager",
            bg=WINDOW_BG, font=("Segoe UI", 18, "bold")
        ).pack(pady=(18, 4))

        tk.Label(
            c, text="Açık X OS pencerelerini yönet.",
            bg=WINDOW_BG, fg="#6b7280"
        ).pack(pady=(0, 10))

        listbox = tk.Listbox(c, font=("Segoe UI", 10))
        listbox.pack(expand=True, fill="both", padx=18, pady=8)

        windows = []

        def title_of(item):
            try:
                labels = item.title_bar.winfo_children()
                if labels:
                    return str(labels[0].cget("text"))
            except Exception:
                pass
            return "X OS uygulaması"

        def refresh():
            listbox.delete(0, "end")
            windows.clear()
            try:
                children = self.desktop.winfo_children()
            except tk.TclError:
                children = []

            for item in children:
                if isinstance(item, InternalWindow) and item.winfo_exists():
                    windows.append(item)
                    listbox.insert(
                        "end",
                        f"{len(windows)}. {title_of(item)}"
                    )
        ...
        tk.Button(
            buttons, text="🔄 Yenile", command=refresh,
            bg=ACCENT, fg="white", relief="flat", padx=12, pady=6
        ).pack(side="left", padx=4)

        tk.Button(
            buttons, text="⬆ Öne Getir", command=focus_selected,
            bg="#374151", fg="white", relief="flat", padx=12, pady=6
        ).pack(side="left", padx=4)

        tk.Button(
            buttons, text="✕ Görevi Kapat", command=close_selected,
            bg="#7f1d1d", fg="white", relief="flat", padx=12, pady=6
        ).pack(side="left", padx=4)
    ...
    def add_notification(self, message):
        ...
        self.notifications.insert(
            0,
            f"{datetime.datetime.now().strftime('%H:%M')} — {message}"
        )
        self.notifications = self.notifications[:20]

    def clear_notifications(self):
        self.notifications = []
        self.add_notification("Bildirim merkezi temizlendi.")

    def open_notification_center(self):
        win = self._new_panel("🔔 Bildirim Merkezi", 430, 390)
        c = win.content

        tk.Label(
            c, text="🔔 Bildirim Merkezi",
            bg=WINDOW_BG, font=("Segoe UI", 18, "bold")
        ).pack(pady=(18, 4))

        box = tk.Listbox(c, font=("Segoe UI", 10))
        box.pack(expand=True, fill="both", padx=18, pady=10)

        for item in getattr(self, "notifications", []):
            box.insert("end", item)

        if not self.notifications:
            box.insert("end", "Yeni bildirim yok.")
    ...
    def lock_xos(self):
        if hasattr(self, "lock_overlay"):
            try:
                if self.lock_overlay.winfo_exists():
                    return
            except tk.TclError:
                pass

        self.lock_overlay = tk.Frame(self, bg="#05070b")
        self.lock_overlay.place(x=0, y=0, relwidth=1, relheight=1)
        self.lock_overlay.lift()

        card = tk.Frame(
            self.lock_overlay, bg="#111827",
            highlightthickness=1,
            highlightbackground="#374151"
        )
        ...
        clock = tk.Label(
            card, text="", bg="#111827", fg="white",
            font=("Segoe UI", 38, "bold")
        )
        clock.pack(pady=(35, 4))

        date = tk.Label(
            card, text="", bg="#111827", fg="#9ca3af",
            font=("Segoe UI", 11)
        )
        date.pack()

        tk.Label(
            card, text="X OS kilitli",
            bg="#111827", fg="#d1d5db",
            font=("Segoe UI", 13, "bold")
        ).pack(pady=16)
        ...
        def tick():
            try:
                if not self.lock_overlay.winfo_exists():
                    return
                now = datetime.datetime.now()
    ...
    def open_xpaint(self):
        win = self._new_panel("🎨 XPaint", 650, 500)
        c = win.content

        toolbar = tk.Frame(c, bg=WINDOW_BG)
        toolbar.pack(fill="x", padx=6, pady=6)

        canvas = tk.Canvas(
            c, bg="white", cursor="crosshair",
            highlightthickness=1,
            highlightbackground="#9ca3af"
        )
        canvas.pack(expand=True, fill="both", padx=6, pady=(0, 6))

        color = tk.StringVar(value="#111827")
        size = tk.IntVar(value=4)
        last = {"x": None, "y": None}
        ...
        tk.Button(
            toolbar, text="🎨 Renk", command=choose_color,
            bg="#e5e7eb", relief="flat", padx=10
        ).pack(side="left", padx=3)

        tk.Label(
            toolbar, text="Boyut:", bg=WINDOW_BG
        ).pack(side="left", padx=(10, 3))

        tk.Spinbox(
            toolbar, from_=1, to=30, width=4,
            textvariable=size
        ).pack(side="left")

        tk.Button(
            toolbar, text="🧹 Temizle", command=clear,
            bg="#e5e7eb", relief="flat", padx=10
        ).pack(side="right", padx=3)

        tk.Label(
            toolbar, text="Fare ile çiz", bg=WINDOW_BG,
            fg="#6b7280"
    ...
    def open_xstore(self):
        win = self._new_panel("🛒 XStore", 560, 470)
        c = win.content
        ...
        tk.Label(
            c,
            text="X OS uygulama mağazası — yerleşik uygulamalar",
            bg=WINDOW_BG, fg="#6b7280"
        ).pack(pady=(0, 12))

        catalog = [
            ("⚙️", "XSettings", "Tema ve görünüm ayarları", self.open_xsettings),
            ("🖥️", "XTaskManager", "Açık pencereleri yönet", self.open_task_manager),
            ("🎨", "XPaint", "Basit çizim uygulaması", self.open_xpaint),
            ("🔔", "Bildirim Merkezi", "X OS bildirimlerini görüntüle",
             self.open_notification_center),
            ("🔒", "Kilit Ekranı", "X OS'u kilitle", self.lock_xos),
        ]

        container = tk.Frame(c, bg=WINDOW_BG)
        container.pack(fill="both", expand=True, padx=18)

        for emoji, name, desc, command in catalog:
            row = tk.Frame(
                container, bg="white",
                highlightthickness=1,
            ...
            tk.Label(
                info, text=name, bg="white",
                font=("Segoe UI", 10, "bold")
            ).pack(anchor="w")
            tk.Label(
                info, text=desc, bg="white",
                fg="#6b7280", font=("Segoe UI", 8)
            ).pack(anchor="w")

            installed = self.store_installed.get(name, True)

            def launch_app(cmd=command):
                win.animate_close(lambda: self.after(30, cmd))

            tk.Button(
                row,
                text="Aç" if installed else "Kur",
                command=launch_app,
                bg=ACCENT if installed else "#374151",
        ...
        tk.Label(
            c,
            text="XStore şu anda X OS'un yerleşik uygulamalarını yönetir.",
            bg=WINDOW_BG, fg="#9ca3af",
            font=("Segoe UI", 8)
        ).pack(pady=8)
